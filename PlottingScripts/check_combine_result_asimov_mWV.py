...
def plot(w,fitres,normset,ch,region):
    ...
    #if region=='sig':
    model.plotOn(p,RooFit.Name("WWWZ_atgc"),RooFit.Normalization(model_norm,RooAbsReal.NumEvent),RooFit.FillColor(colors["atgc"]),RooFit.LineStyle(9),RooFit.LineWidth(2))
    ...
    data_histo   = data.binnedClone("data","data").createHistogram("data",rrv_x,RooFit.Cut("CMS_channel==CMS_channel::%s"%ch_num),RooFit.Binning(36,900,4500))
    data_histo.Print()
...
def plot_all(w,ch="el",reg='sig'):
    ...
    if ch=='el':
        ...
        legMWV.AddEntry(p.getObject(12),"Asimov data","PE")
    else:
