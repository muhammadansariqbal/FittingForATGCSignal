...
def plot(ch="el"):
	...
	# Draw the histograms
	treeIso.Draw("jet_mass_softdrop_PUPPI >> histIso(55,0,165)", "(250) * (jet_mass_softdrop_PUPPI>0 && jet_mass_softdrop_PUPPI<160 && (jet_mass_softdrop_PUPPI<40. || jet_mass_softdrop_PUPPI>150.) && MWW_SD>900)", "HIST")
        histIso = gDirectory.Get("histIso")

	treeMC.Draw("jet_mass_softdrop_PUPPI >> histMC(55,0,165)", "(250*totEventWeight) * (jet_mass_softdrop_PUPPI>0 && jet_mass_softdrop_PUPPI<160 && (jet_mass_softdrop_PUPPI<40. || jet_mass_softdrop_PUPPI>150.) && MWW_SD>900)", "HIST SAME")
        histMC = gDirectory.Get("histMC")

	treeAntiIso.Draw("jet_mass_softdrop_PUPPI >> histAntiIso(55,0,165)", "(1) * (jet_mass_softdrop_PUPPI>0 && jet_mass_softdrop_PUPPI<160 && (jet_mass_softdrop_PUPPI<40. || jet_mass_softdrop_PUPPI>150.) && nbtag==0)", "HIST")
	...
	# Set properties
	histAntiIso.SetLineColor(kViolet+1)
	...
	leg.AddEntry(histIso,"QCD isolated side-sideband #times 250","L")
	leg.Draw()
	legends.append(leg)
