...
parser.add_option('--axes',dest='axes',default='0',help='Axes ranges xlo,xhi,ylo,yhi')
...
def plots():
	...
	if float(axes[0])==0:
		...
		Xaxis = contourExp99.GetXaxis()
		Xaxis.SetLimits(float(axes[0]),float(axes[1]))
		contourExp99.GetYaxis().SetRangeUser(float(axes[2]),float(axes[3]))
	...
	leg.AddEntry(contourExp99,"8 TeV 95% CL","L")
