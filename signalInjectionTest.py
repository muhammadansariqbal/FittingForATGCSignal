...
class Prepare_workspace_4limit:
        ...
        def Make_plots(self,rrv_x,cat,fitres,poi='ccw'):
            ...
            ratio_style.GetYaxis().SetTitle('#frac{MC-Model}{#sigma_{MC}}  ')
            ...
            canvas = TCanvas(channel,channel,800,640)
            ...
            # Main MWV plot
            p	= rrv_x.frame(RooFit.Bins(rrv_x.getBins()),RooFit.Title(self.ch))
            if self.ch=='el':
...
#            data_SM_histo	= self.wtmp.data('SMdatahist_%s'%cat).createHistogram("data_SM",rrv_x)
#            data_SM	= RooHist(data_SM_histo,rrv_x.getBinWidth(0))
#            data_SM.SetMarkerStyle(20)
#            data_SM.SetMarkerColor(kGreen+3)
#            data_SM.SetLineColor(kGreen+3)
#            alpha        = 1-0.6827
#            for iPoint in range(data_SM.GetN()):
#                N = data_SM.GetY()[iPoint]
#                if N==0 :
#                    L = 0
#                    U = 0
#                else:
...
#                data_SM.SetPointEYlow(iPoint, N-L)
#                data_SM.SetPointEYhigh(iPoint, U-N)
#                data_SM.SetPointEXlow(iPoint,0)
#                data_SM.SetPointEXhigh(iPoint,0)
#            data_SM.SetName('data_SM')
#            p.addPlotable(data_SM,"PE")
...
#            data_cMax.SetMarkerColor(kBlue+2)
#            data_cMax.SetLineColor(kBlue+2)
#            alpha        = 1-0.6827
#            for iPoint in range(data_cMax.GetN()):
#                N = data_cMax.GetY()[iPoint]
...
#                    L = Math.gamma_quantile(alpha/2.,N,1.)
#                    U = Math.gamma_quantile_c(alpha/2,N+1,1.)
#                data_cMax.SetPointEYlow(iPoint, N-L)
#                data_cMax.SetPointEYhigh(iPoint, U-N)
#                data_cMax.SetPointEXlow(iPoint,0)
#                data_cMax.SetPointEXhigh(iPoint,0)
#            data_cMax.SetName('data_cMax')
...
#            data_cTest.SetMarkerColor(kRed+2)
#            data_cTest.SetLineColor(kRed+2)
#            alpha        = 1-0.6827
#            for iPoint in range(data_cTest.GetN()):
#                N = data_cTest.GetY()[iPoint]
#                if N==0 :
#                    L = 0
#                    U = 0
#                else:
#                    L = Math.gamma_quantile(alpha/2.,N,1.)
#                    U = Math.gamma_quantile_c(alpha/2,N+1,1.)
#                data_cTest.SetPointEYlow(iPoint, N-L)
#                data_cTest.SetPointEYhigh(iPoint, U-N)
#                data_cTest.SetPointEXlow(iPoint,0)
#                data_cTest.SetPointEXhigh(iPoint,0)
#            data_cTest.SetName('data_cTest')
#            p.addPlotable(data_cTest,"PE")
        
            # Cosmetics
            p.GetXaxis().SetTitleSize(0)
            p.GetXaxis().SetLabelSize(0)
            p.GetYaxis().SetRangeUser(3e-3,1e5)
            ...
            # mSD range text
            pt3 = TPaveText(0.132,0.725,0.325,0.9, "blNDC")
            pt3.SetFillStyle(0)
            pt3.SetBorderSize(0)
            pt3.SetTextAlign(13)
            pt3.SetTextSize(0.06)
            if cat=="WW":
            	pt3.AddText("WW category")
            else:
