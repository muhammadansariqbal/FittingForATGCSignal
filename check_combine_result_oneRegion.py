...
def plot(w,fitres,normset,spectrum,ch,region):
    ...
    if spectrum == "mj":
        ...
        #w.var("cwww").setVal(0);w.var("ccw").setVal(0);w.var("cb").setVal(0);
        model_norm_tmp = float(bkg_norms["WJets"].getVal()+bkg_norms["STop"].getVal()+bkg_norms["TTbar"].getVal()+bkg_norms["WW"].getVal()+bkg_norms["WZ"].getVal())
        model.plotOn(p,RooFit.Name("WJets"),RooFit.Components("STop,WJets,TTbar,WW,WZ"),RooFit.Normalization(model_norm_tmp,RooAbsReal.NumEvent),RooFit.FillColor(colors["WJets"]),RooFit.LineColor(kBlack),RooFit.DrawOption("F"))
        model.plotOn(p,RooFit.Name("TTbar"),RooFit.Components("STop,TTbar,WW,WZ"),RooFit.Normalization(model_norm_tmp,RooAbsReal.NumEvent),RooFit.FillColor(kOrange),RooFit.LineColor(kBlack),RooFit.DrawOption("F"))
        ...
        model.plotOn(p,RooFit.Name("WWSM"),RooFit.Components("WW,WZ,STop"),RooFit.Normalization(model_norm_tmp,RooAbsReal.NumEvent),RooFit.FillColor(colors["WW"]),RooFit.LineColor(kBlack),RooFit.DrawOption("F"))
        model.plotOn(p,RooFit.Name("WZSM"),RooFit.Components("WZ,STop"),RooFit.Normalization(model_norm_tmp,RooAbsReal.NumEvent),RooFit.FillColor(colors["WZ"]),RooFit.LineColor(kBlack),RooFit.DrawOption("F"))
        ...
        model.plotOn(p,RooFit.Name("STop"),RooFit.Components("STop"),RooFit.Normalization(model_norm,RooAbsReal.NumEvent),RooFit.FillColor(colors["STop"]),RooFit.LineColor(kBlack),RooFit.DrawOption("F"))
...
def plot_all(w,ch="el",name='test.png',reg='sig'):
    ...
    leg.SetTextFont(42)
    if reg!='sig':
        #leg.AddEntry(p.getObject(10),"CMS data, W#rightarrow #mu#nu","P")
        leg.AddEntry(p.getObject(10),"CMS data","P")
    leg.AddEntry(p.getObject(0),"W+jets","F")
    leg.AddEntry(p.getObject(1),"t#bar{t}","F")
    leg.AddEntry(p.getObject(4),"WW","F")
    leg.AddEntry(p.getObject(5),"WZ","F")
    leg.AddEntry(p.getObject(8),"Single top","F")
    leg.Draw()
...
plot_all(w,options.ch,'postfit_%s_sig.pdf'%options.ch,'sig')
